...
double getD2FitMuon(double *par, double phi, double eta, double z)
{
...
XYZVector convertPhiEtaZ_XYZ(double phi, double eta, double z)
{
    double theta = 2*TMath::ATan(exp(-1.*eta));
    double rho = fabs(z*TMath::Tan(theta));
    double y = rho*TMath::Sin(phi);
...
// calculate distance line-point
double getD2GenMuon(XYZVector xp, XYZVector x0, XYZVector x1)
{
    // distance line point is D= | (xp-x0) cross  ux |
    // where ux is direction of line and x0 is a point in the line (like t = 0)
    XYZVector u = (x1-x0).Unit();
    double distance = ((xp-x0).Cross(u)).Mag2();
    return sqrt(distance);
}
...
dofit(std::vector<double> allphiG, std::vector<double> alletaG, std::vector<double> allzG,
	...
	XYZVector genMuonP0, XYZVector genMuonP1 )
{
	...
	cout << "amin: " << amin << endl;
        ...
        double d_fitMuon = getD2FitMuon( parFit,allphiG[lct],alletaG[lct],allzG[lct] );
	double dr0 = sqrt(distance2(0.,0.,0.,parFit));


	XYZVector toCalPoint = convertPhiEtaZ_XYZ(allphiG[lct],alletaG[lct],allzG[lct]);
	float d_genMuon = getD2GenMuon(toCalPoint,genMuonP0,genMuonP1);
	...
	myfit.strip = allstrip[lct];
        ...
        myfit.p2 = parFit[2];
        myfit.p3 = parFit[3];
        myfit.d_fitMuon = d_fitMuon;
	myfit.dr0 = dr0;
        myfit.chi2 = amin;
	myfit.d_genMuon = d_genMuon;
...
void readLCTAnalyzer_MC_OneHitperStation(TString _input_="./csctf_Run246926.root", TString _output_="output_test.root", int entry0=0)
{
    ...
    t_->SetBranchAddress("nmcparticles", &nmcparticles);
    t_->SetBranchAddress("mc_px", mc_px);
    t_->SetBranchAddress("mc_py", mc_py);
    t_->SetBranchAddress("mc_pz", mc_pz);
    t_->SetBranchAddress("mc_vx", mc_vx);
    t_->SetBranchAddress("mc_vy", mc_vy);
    t_->SetBranchAddress("mc_vz", mc_vz);
    t_->SetBranchAddress("mc_en", mc_en);
    t_->SetBranchAddress("mc_id", mc_id);
    t_->SetBranchAddress("mc_status", mc_status);
    ...
    TH1F* h_m_dr0 = new TH1F("h_m_dr0",";Distance to IP [mm], ME-;Events",500,0,100);
    TH1F* h_p_dr0 = new TH1F("h_p_dr0",";Distance to IP [mm], ME+;Events",500,0,100);
    ...
    TH1F *h_m_dFitMuon = new TH1F("h_m_dFitMuon",";Distance [mm], ME-;Events",200,0,200.);
    ...
    TH1F *h_p_dFitMuon = new TH1F("h_p_dFitMuon",";Distance, Plus EndCap;Events",200,0,200.);


    TH1F *h_p_dGenMuon = new TH1F("h_p_dGenMuon",";Distance to Gen Muon, Plus EndCap;Events",200,0,200.);
	...
	if(nmcparticles!=1) continue;
	XYZVector genMuonP0(mc_vx[0],mc_vy[0],mc_vz[0]);
	XYZVector genMuonP1(mc_px[0]+mc_vx[0],mc_py[0]+mc_vy[0],mc_pz[0]+mc_vz[0]);
	TLorentzVector genMuon;
	genMuon.SetPxPyPzE(mc_px[0],mc_py[0],mc_pz[0],mc_en[0]);
	float genPhi = genMuon.Phi();
	float genEta = genMuon.Eta();
	    ...
	    if(diffstation.size()<2) continue;
            ...
            std::vector<CSCTF_t> deltaPhiEtas = dofit(allphiG,alletaG,allzG,allendcap,allsector,allstation,allring,allcscid,allstrip,allwire,allsubsector,genMuonP0,genMuonP1);
		...
		int strip = deltaPhiEtas[j].strip;
                ...
                float d_fitMuon = deltaPhiEtas[j].d_fitMuon;
		float dr0 = deltaPhiEtas[j].dr0;
		float chi2 = deltaPhiEtas[j].chi2;

		float d_genMuon = deltaPhiEtas[j].d_genMuon;
		float dphi_gen = deltaPhi(rawphi,genPhi);
		float deta_gen = raweta - genEta;

		//if(chi2>0.1) continue;

                //if( fabs(p0)>200 || fabs(p1)>200 || fabs(p2)>200 || fabs(p3)>200 || chi2>0.5 ||  dr0 > 500) continue;
		if( chi2>0.05 || dr0>23) continue;
		    ...
		    h_m_chi2 -> Fill(chi2);
                    ...
                    h_p_p2 ->Fill(p2);
                    h_p_p3 ->Fill(p3);
                    h_p_dFitMuon->Fill(d_fitMuon);
		    h_p_dr0 -> Fill(dr0);
		    h_p_chi2 -> Fill(chi2);
		    h_p_dGenMuon->Fill(d_genMuon);
                }
		...
		outfile << " endcap: " << endcap
			...
			<< endl;
                ...
                outfile << "status: " << std::setprecision(1) << status << "\t"
                        ...
                        << " d_fitMuon: " << std::setprecision(5) << d_fitMuon
                        ;
		...
		if(endcap==1){
			...
			h_deltaGenPhiMEp42 -> Fill(dphi_gen);
			h_deltaGenEtaMEp42 -> Fill(deta_gen);
                    }
                    if(endcap == 1) {
